...
function isNodeRuntime() {
    var _a25;
    return typeof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"] !== "undefined" && ((_a25 = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].release) == null ? void 0 : _a25.name) === "node";
}
...
function hasToolCall(...toolName) {
    return ({ steps })=>{
        var _a25, _b25, _c;
        return (_c = (_b25 = (_a25 = steps[steps.length - 1]) == null ? void 0 : _a25.toolCalls) == null ? void 0 : _b25.some((toolCall)=>toolName.includes(toolCall.toolName))) != null ? _c : false;
    };
}
                ...
                try {
                    await runInTracingChannelSpan({
                        ...
                        execute: async ()=>{
                            ...
                            if (enforcedToolChoice != null && !stepToolCalls.some((toolCall)=>enforcedToolChoice.type === "required" || toolCall.toolName === enforcedToolChoice.toolName)) {
...
function handleUIMessageStreamFinish({ messageId, originalMessages = [], onStepEnd, onStepFinish, onEnd, onFinish, onError, stream, getOutcome }) {
    ...
    const callOnEnd = async ()=>{
        ...
        const declaredOutcome = (_a25 = getOutcome == null ? void 0 : getOutcome()) != null ? _a25 : {
            status: "unknown"
        };
...
function createLanguageModelV4StreamPartToLanguageModelStreamPartTransform({ tools, instructions, messages, repairToolCall, refineToolInput, callId, provider, modelId, toolChoice, generateId: generateId4, now: now2, callStartTimestampMs, onLanguageModelCallEnd }) {
                ...
                case "finish":
                    {
                        ...
                        if (enforcedToolChoice != null && ![
                            ...toolCallsByToolCallId.values()
                        ].some((toolCall)=>enforcedToolChoice.type === "required" || toolCall.toolName === enforcedToolChoice.toolName)) {
                            controller.enqueue({
                                type: "error",
...
function applyStreamTextTransforms({ stream, transforms, tools, stopStream }) {
    ...
    const createTransformedSegmentReader = ()=>{
        ...
        for (const transform of transforms){
            segment = segment.pipeThrough(transform({
                tools,
                stopStream
            }));
        }
                        ...
                        async pull (controller) {
                            ...
                            while(true){
                                ...
                                if (enqueueStreamRetryAttemptBoundary) {
                                    controller.enqueue(createStreamRetryAttemptBoundaryPart({
                                ...
                                const callbackRetry = !isToolChoiceViolation && !automaticRetry && callbackRequestedRetry && callbackStreamRetryCount < 1;
...
var ToolLoopAgent = class {
   ...
   */ get tools() {
    ...
    async prepareCall(options) {
        ...
        const preparedCallArgs = (_d = await ((_c = (_b25 = this.settings).prepareCall) == null ? void 0 : _c.call(_b25, baseCallArgs))) != null ? _d : baseCallArgs;
   ...
   */ async generate({ abortSignal, timeout, experimental_sandbox: sandbox, onStart, experimental_onStart, onStepStart, experimental_onStepStart, onToolExecutionStart, onToolExecutionEnd, onStepEnd, onStepFinish, onFinish, onEnd = onFinish, ...options }) {
        ...
        const callbackArgs = {
            ...
            experimental_sandbox: sandbox,
            onStart: mergeCallbacks((_a25 = this.settings.onStart) != null ? _a25 : this.settings.experimental_onStart, onStart != null ? onStart : experimental_onStart),
            onStepStart: mergeCallbacks((_b25 = this.settings.onStepStart) != null ? _b25 : this.settings.experimental_onStepStart, onStepStart != null ? onStepStart : experimental_onStepStart),
...
async function startBatch({ provider, requests, providerOptions, webhookUrl, abortSignal, headers, timeout }) {
    ...
    try {
        ...
        const modelByRequestId = new Map(normalizedRequests.map((request)=>[
                request.id,
                request.modelId
            ]));
...
function validateCompatibleTools({ requestId, tools, toolsByName }) {
    for (const tool2 of tools != null ? tools : []){
        const previousTool = toolsByName.get(tool2.name);
        if (previousTool != null && !isDeepEqualData(previousTool, tool2)) {
            throw new InvalidArgumentError({
                parameter: "requests",
                value: requestId,
                message: `tool "${tool2.name}" must have the same definition in every batch request`
            });
        }
        toolsByName.set(tool2.name, tool2);
    }
}
async function getBatchStatus({ provider, batch, providerOptions, maxRetries, abortSignal, headers, timeout }) {
    ...
    try {
        const status = await retry(()=>batchApi.doGetBatchStatus({
                batchId: batch.id,
                providerOptions,
                abortSignal: operationAbortSignal,
                headers: (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$ai$2d$sdk$2f$provider$2d$utils$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$locals$3e$__["withUserAgentSuffix"])(headers != null ? headers : {}, `ai/${VERSION}`)
...
function getBatchResults({ provider, batch, tools, providerOptions, maxRetries, abortSignal, headers, timeout }) {
    ...
    void (async ()=>{
        try {
            const stream = await retry(()=>batchApi.doGetBatchResults({
                    batchId: batch.id,
                    providerOptions,
                    abortSignal: operationAbortSignal,
                    headers: (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$ai$2d$sdk$2f$provider$2d$utils$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$locals$3e$__["withUserAgentSuffix"])(headers != null ? headers : {}, `ai/${VERSION}`)
...
async function convertBatchItemResult({ item, tools }) {
    switch(item.type){
        case "text":
            switch(item.status){
                case "succeeded":
        ...
        execute: async ()=>{
            ...
            try {
                const { embedding, usage, warnings, response, providerMetadata } = await retry(async ()=>{
                    ...
                    const usage2 = (_a27 = modelResponse.usage) != null ? _a27 : {
                        tokens: NaN
                    };
                    await notify({
                        event: {
    ...
    return await runInTracingChannelSpan({
        ...
        execute: async ()=>{
            ...
            try {
                ...
                for (const parallelChunk of parallelChunks){
                    const results = await Promise.all(parallelChunk.map(async (chunk)=>{
                        const result = await retry(async ()=>{
                            ...
                            const usage = (_a27 = modelResponse.usage) != null ? _a27 : {
                                tokens: NaN
                            };
                            await notify({
                                event: {
                ...
                await ((_b25 = telemetryDispatcher.onError) == null ? void 0 : _b25.call(telemetryDispatcher, {
                    callId,
                    error
                }));
                throw error;
...
async function executeStartStatusFlow({ model, callOptions, poll: pollConfig, webhook: webhookFactory, retry }) {
    ...
    const callerIdempotencyKey = Object.entries((_a25 = callOptions.headers) != null ? _a25 : {}).find(([key, value])=>key.toLowerCase() === "idempotency-key" && value !== void 0);
...
function normalizeVideoCallInputs({ promptArg, frameImages, inputReferences }) {
    ...
    const firstFrameImage = (_a25 = normalizedFrameImages == null ? void 0 : normalizedFrameImages.find((frame)=>frame.frameType === "first_frame")) == null ? void 0 : _a25.image;
...
var doWrap = ({ model, middleware: { transformParams, wrapGenerate, wrapStream, overrideProvider, overrideModelId, overrideSupportedUrls }, modelId, providerId })=>{
    ...
    return {
        specificationVersion: "v4",
        provider: (_a25 = providerId != null ? providerId : overrideProvider == null ? void 0 : overrideProvider({
            model
        })) != null ? _a25 : model.provider,
        modelId: (_b25 = modelId != null ? modelId : overrideModelId == null ? void 0 : overrideModelId({
            model
        })) != null ? _b25 : model.modelId,
...
var doWrap3 = ({ model, middleware: { transformParams, wrapGenerate, overrideProvider, overrideModelId, overrideMaxImagesPerCall }, modelId, providerId })=>{
    ...
    const maxImagesPerCallRaw = (_a25 = overrideMaxImagesPerCall == null ? void 0 : overrideMaxImagesPerCall({
        model
    })) != null ? _a25 : model.maxImagesPerCall;
    const maxImagesPerCall = maxImagesPerCallRaw instanceof Function ? maxImagesPerCallRaw.bind(model) : maxImagesPerCallRaw;
    return {
        specificationVersion: "v4",
        provider: (_b25 = providerId != null ? providerId : overrideProvider == null ? void 0 : overrideProvider({
            model
        })) != null ? _b25 : model.provider,
...
var RealtimeEventReducer = class {
    ...
    addInputTranscriptionMessage(state, itemId, transcript) {
        ...
        const insertIndex = Math.min((_a25 = this.inputAudioMessageInsertIndex.get(itemId)) != null ? _a25 : state.messages.length, state.messages.length);
        const messages = [
            ...state.messages
        ];
...
var AbstractRealtimeSession = class {
    ...
    async connect() {
        ...
        try {
            ...
            this.applyState(this.reducer.setStatus(this.state, "error"));
            (_a25 = this.onError) == null ? void 0 : _a25.call(this, error instanceof Error ? error : new Error(`Connection failed: ${String(error)}`));
        }
    }
    disconnect() {
   ...
   */ maybeRequestToolResponse() {
    ...
    async executeToolCall({ name: name25, args, callId }) {
        var _a25, _b25;
        ...
        try {
            ...
            (_b25 = this.onError) == null ? void 0 : _b25.call(this, error instanceof Error ? error : new Error(`Client tool execution failed: ${String(error)}`));
        }
    }
